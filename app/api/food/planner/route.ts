...
export async function GET(request: Request) {
  try {
    ...
    const budgetVal = queryBudget ? parseFloat(queryBudget) : (user.budget ? Number(user.budget) : 1400.0);
    ...
    // Curated local Indian budget diet fallbacks (in Rupees)
    const isVeg = queryDiet ? queryDiet === "veg" : (user.goal?.toLowerCase().includes("veg") || user.goal === "maintain");
    ...
    // Query Google Gemini AI API if a valid key is provided
...
- Daily Food Budget: Rs. ${Math.round(dailyBudget)} INR
- Diet Preference: ${dietTypeStr}

The meal plan must consist of real Indian foods with affordable pricing in Indian Rupees (INR).
You must output a strict JSON object matching this structure exactly (only return the JSON):
{
  "breakfast": { "name": "string", "calories": number, "protein": number, "cost": number, "ingredients": ["string"] },
  "lunch": { "name": "string", "calories": number, "protein": number, "cost": number, "ingredients": ["string"] },
  "dinner": { "name": "string", "calories": number, "protein": number, "cost": number, "ingredients": ["string"] },
  "snack": { "name": "string", "calories": number, "protein": number, "cost": number, "ingredients": ["string"] }
}`;
        ...
        console.error("Gemini API call error, using fallbacks:", geminiErr);
      }
    }
...
export async function POST(request: Request) {
  try {
    ...
    const { dietPreference = "non-veg", budget = 1400 } = body;
    ...
    const genderVal = user.gender ? user.gender : "male";
    const goalVal = user.goal ? user.goal : "WEIGHT_LOSS";
    const budgetVal = parseFloat(budget.toString());
    ...
    let targetCalories = Math.round(TDEE);
    let targetProtein = Math.round(weightVal * 1.6);

    if (goalVal === "WEIGHT_LOSS") {
      targetCalories = Math.round(TDEE - 500);
      targetProtein = Math.round(weightVal * 1.8);
    } else if (goalVal === "MUSCLE_GAIN") {
      targetCalories = Math.round(TDEE + 300);
      targetProtein = Math.round(weightVal * 2.0);
    }
    ...
    const selectedPlan = isVeg ? vegFallback : nonVegFallback;
    ...
    const targetList = isVeg ? vegGroceryItems : nonVegGroceryItems;
